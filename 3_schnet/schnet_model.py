...
class AtomData2Img64(nn.Module):
    """
    Given preprocessed data of atoms
    return a one-hot vector of size 5
    """
    def __init__(
        self
    ):
        ...
        self.input_layer = SchNet(
            radial_basis = GaussianRBF(n_rbf=60, cutoff=6.0, trainable=False),
            cutoff_fn = CosineCutoff(cutoff=6.0)
            )
        
        # Atom wise layer for down sampling
        self.output_layer = AtomWiseOutput(hidden_layers=[32], n_out_basis=1)
    ...
    def forward(
        ...
        x = self.output_layer(x)
        # max pooling
        x = x["scalar_representation"][-65536:].view(64*64, 16)
